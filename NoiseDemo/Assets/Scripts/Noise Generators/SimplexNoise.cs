...
public class SimplexNoise: INoise {  // Simplex noise in 2D, 3D and 4D
  ...
  private float F2 = 0.5f * (Mathf.Sqrt(3) - 1);
  private float G2 = (3 - Mathf.Sqrt(3)) / 6;
  private float F3 = 1f/3;
  private float G3 = 1f/6;
  private float F4 = (Mathf.Sqrt(5)-1)/4f;
  private float G4 = (5-Mathf.Sqrt(5))/20f;
  ...
  private double dot(Vector2 g, double x, double y) {
    return g.x*x + g.y*y; }
  ...
  public double noise(double xin, double yin) {
    ...
    public float GetNoiseValue(int x, int y) {
        return (float)noise(x / (scale / 10000), y / (scale / 10000));
    }
